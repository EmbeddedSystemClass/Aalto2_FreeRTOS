...
#define USE_QUEUES_SCI
#define QUEUE_WAIT 20
#define BUFFER_LENGTH 150

QueueHandle_t sciTQ[2];
QueueHandle_t sciRQ[2];

SemaphoreHandle_t sciM[2];
...
/* USER CODE BEGIN (3) */
    ...
    sciTQ[0] = xQueueCreate(BUFFER_LENGTH, sizeof(uint8));
    sciTQ[1] = xQueueCreate(BUFFER_LENGTH, sizeof(uint8));
    sciRQ[0] = xQueueCreate(BUFFER_LENGTH, sizeof(uint8));
    sciRQ[1] = xQueueCreate(BUFFER_LENGTH, sizeof(uint8));
    sciM[0] = xSemaphoreCreateMutex();
    sciM[1] = xSemaphoreCreateMutex();
	...
	switch (vec)
	    ...
	    case 11U:
			/* receive */
				...
				if(xQueueIsQueueFullFromISR(sciRQ[0U]) == pdFALSE)
				{
					xQueueSendToBackFromISR(sciRQ[0U], &byte, NULL);
				}
				else
				{
		...
		case 12U:
			...
			if(uxQueueMessagesWaitingFromISR(sciTQ[0U]) > 0)
			{
				uint8 txdata;
				xQueueReceiveFromISR(sciTQ[0U], &txdata, NULL);
	...
	switch (vec)
	    ...
	    case 11U:
	        /* receive */
				...
				if(xQueueIsQueueFullFromISR(sciRQ[1U]) == pdFALSE)
				{
					xQueueSendToBackFromISR(sciRQ[1U], &byte, NULL);
				}
				else
				{
	    ...
	    case 12U:
	    	...
	    	if(uxQueueMessagesWaitingFromISR(sciTQ[1U]) > 0)
	    	{
	    		uint8 txdata;
	    		xQueueReceiveFromISR(sciTQ[1U], &txdata, NULL);
...
/** @fn uint32 sciSendByteQ(sciBASE_t *sci, uint8 byte)
*   @brief Send Byte through queue, thread safe
*   @param[in] sci  - sci module base address
*   @param[in] byte - byte to transfer
*
*   @return error code, pdTRUE or pdFALSE
*
*   Sends a single byte through FreeRTOS queue.
*/
BaseType_t sciSendByteOS(sciBASE_t *sci, uint8 byte)
{
        uint32 index = (sci == sciREG) ? 0U : 1U;

	if ((g_sciTransfer_t[index].mode & (uint32)SCI_TX_INT) != 0U)
	{
	    if(uxQueueMessagesWaiting(sciTQ[index]) == 0)
	    {
	    	sci->TD     = (uint32)(byte);
	    	sci->SETINT = (uint32)SCI_TX_INT;
	    }
	    ...
	    {
	    	if(xQueueSendToBack(sciTQ[index], &byte, QUEUE_WAIT) == errQUEUE_FULL)
	    	{
	    		return pdFALSE;
	    	}
	    }
...
/** @fn void sciSendQ(sciBASE_t *sci, uint32 length, uint8 * data)
*   @brief Send Data
*   @param[in] sci    - sci module base address
*   @param[in] length - number of data words to transfer
*   @param[in] data   - pointer to data to send
...
*
*   Send a block of data pointed to by 'data' and 'length' bytes
*   long. Uses FreeRTOS queues for thread safety. Requires interupts
*   to be ebabled.
*
*   @note if data word is less than 8 bits, then the data must be left
*         aligned in the data byte.
*/
BaseType_t sciSendOS(sciBASE_t *sci, uint32 length, uint8 * data)
{
    int i;
    int message_waiting;
    uint32 index = (sci == sciREG) ? 0U : 1U;
...
/** @fn uint32 sciReceiveByteQ(sciBASE_t *sci)
*   @brief Receive Byte
*   @param[in] sci - sci module base address
*
*   @return error code, pdTRUE or pdFALSE
*
*    Receives a single byte through FreeRTOS queue. 
*/
BaseType_t  sciReceiveByteOS(sciBASE_t *sci, uint8 * byte)
{
	uint32 index = (sci == sciREG) ? 0U : 1U;
	if ((sci->SETINT & (uint32)SCI_RX_INT) == (uint32)SCI_RX_INT)
	{
	    return xQueueReceive(sciRQ[index], byte, QUEUE_WAIT);
	}
	return pdFALSE;
}

/** @fn void sciReceiveQ(sciBASE_t *sci, uint32 length, uint8 * data)
*   @brief Receive Data
*   @param[in] sci    - sci module base address
*   @param[in] length - number of data words to transfer
*   @param[in] data   - pointer to data buffer to receive data
...
*
*   Receive a block of 'length' bytes long and place it into the 
*   data buffer pointed to by 'data'. Uses FreeRTOS queues for thread
*   safety. Requires interrupts to be enabled.
*/
BaseType_t sciReceiveOS(sciBASE_t *sci, uint32 length, uint8 * data)
{
    int i;
    uint32 index = (sci == sciREG) ? 0U : 1U;
    ...
    if ((sci->SETINT & (uint32)SCI_RX_INT) == (uint32)SCI_RX_INT)
    {
        ...
        for(i = 0; i < length; i++)
        {
        	if(xQueueReceive(sciRQ[index], data, QUEUE_WAIT) == pdFALSE)
        	{
        		xSemaphoreGive(sciM[index]);
        		return pdFALSE;
        	}
